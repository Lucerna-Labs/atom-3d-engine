"""
Generate fully fleshed-out training memories for LoRA training.

The simulation proved the architecture works. Now we need real episodic
content — not templates with placeholders, but actual scene-based memories
that a model can learn from.

Each memory is a first-person scene where the skill was EXPERIENCED.
The model should feel like it has BEEN THERE.
"""
...
ALL_MEMORIES = FORENSIC_MEMORIES + LOGIC_MEMORIES

# Now generate additional memories to fill out the corpus to ~228
# using the template structure but with full content
...
ALL_MEMORIES.extend(ADDITIONAL_FORENSIC)
ALL_MEMORIES.extend(ADDITIONAL_LOGIC)

# Add more memories to reach ~228
# Generate additional memories for each behavior to ensure full coverage
...
ALL_MEMORIES.extend(EXTRA_MEMORIES)

print(f"Total memories: {len(ALL_MEMORIES)}")
print(f"Forensic: {sum(1 for m in ALL_MEMORIES if m['domain'] == 'forensic')}")
print(f"Logic: {sum(1 for m in ALL_MEMORIES if m['domain'] == 'logic')}")

# Check behavior coverage
all_behaviors = set()
for m in ALL_MEMORIES:
    all_behaviors.update(m['behaviors_encoded'])
print(f"Unique behaviors: {len(all_behaviors)}")
print(f"Behaviors: {sorted(all_behaviors)}")

# Save the corpus
output_path = Path("F:/OPENCLAW-PROJECTS/lora-memory-experiment/corpus/final_fl/training_corpus_fl.json")
