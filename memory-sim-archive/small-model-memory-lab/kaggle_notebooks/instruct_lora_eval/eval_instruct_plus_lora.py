#!/usr/bin/env python
"""Evaluate an instruct model with the trained cyber-memory LoRA attached."""
...
WORKING_DIR = Path("/kaggle/working") if Path("/kaggle/working").exists() else Path.cwd() / "runs" / "kaggle-instruct-lora-local"
WORKING_DIR.mkdir(parents=True, exist_ok=True)
REPORT_PATH = WORKING_DIR / "instruct_plus_lora_eval_results.json"
...
def install_if_needed() -> None:
    packages = ["torch", "transformers", "accelerate", "peft"]
    missing = []
...
def find_adapter_dir() -> Path:
    ...
    for base in [Path("/kaggle/input"), Path.cwd() / "runs" / "kaggle-lora-v4"]:
        if base.exists():
            candidates.extend(base.rglob("adapter_config.json"))
    ...
    raise FileNotFoundError(
        "Could not find cyber-memory LoRA adapter. Expected adapter_config.json "
        "and adapter_model.safetensors under /kaggle/input."
    )


def load_model_tokenizer_adapter() -> tuple[Any, Any, Path]:
    import torch
...
def main() -> int:
    install_if_needed()
    model, tokenizer, adapter_dir = load_model_tokenizer_adapter()
    rows = []
