#!/usr/bin/env python
"""Train and evaluate a small LoRA on the Rust cyber-defender memory corpus.

Default behavior is a safe setup pass: validate the dataset, build examples,
write previews, and stop before training. Flip TRAINING_ENABLED to True when
you are ready to spend Kaggle GPU time.
"""
...
# Qwen3-1.7B-Base is the known HF/PyTorch base-model path used in this lab.
# Override with BASE_MODEL_ID if you want to try another HF-format base model.
...
OUTPUT_DIR = WORKING_DIR / "rust-cyber-memory-lora"
REPORT_PATH = WORKING_DIR / "lora_eval_results.json"
PREVIEW_PATH = WORKING_DIR / "training_examples_preview.json"
...
def install_if_needed() -> None:
    ...
    if gpu_capability.startswith("6.") and os.environ.get("P100_TORCH_RESTARTED") != "1":
        print("Detected a P100-class GPU. Installing a PyTorch CUDA wheel that still supports sm_60.")
        subprocess.check_call(
            [
                sys.executable,
                "-m",
